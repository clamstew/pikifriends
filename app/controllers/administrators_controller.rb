class AdministratorsController < ApplicationController
  
  protect_from_forgery
  before_filter :login_required, :except => [:login,:authorize]
  
  # GET /administrators
  # GET /administrators.xml
  def index
    @administrators = Administrator.all
    
    respond_to do |format|
      format.html # index.html.erb
      format.xml  { render :xml => @administrators }
    end
  end

  # GET /administrators/1
  # GET /administrators/1.xml
  def show
    @administrator = Administrator.find(params[:id])

    respond_to do |format|
      format.html # show.html.erb
      format.xml  { render :xml => @administrator }
    end
  end

  # GET /administrators/new
  # GET /administrators/new.xml
  def new
    @administrator = Administrator.new

    respond_to do |format|
      format.html # new.html.erb
      format.xml  { render :xml => @administrator }
    end
  end

  # GET /administrators/1/edit
  def edit
    @administrator = Administrator.find(params[:id])
  end

  # POST /administrators
  # POST /administrators.xml
  def create
    @administrator = Administrator.new(params[:administrator])

    respond_to do |format|
      if @administrator.save
        flash[:notice] = 'Administrator was successfully created.'
        format.html { redirect_to(@administrator) }
        format.xml  { render :xml => @administrator, :status => :created, :location => @administrator }
      else
        format.html { render :action => "new" }
        format.xml  { render :xml => @administrator.errors, :status => :unprocessable_entity }
      end
    end
  end

  # PUT /administrators/1
  # PUT /administrators/1.xml
  def update
    @administrator = Administrator.find(params[:id])

    respond_to do |format|
      if @administrator.update_attributes(params[:administrator])
        flash[:notice] = 'Administrator was successfully updated.'
        format.html { redirect_to(@administrator) }
        format.xml  { head :ok }
      else
        format.html { render :action => "edit" }
        format.xml  { render :xml => @administrator.errors, :status => :unprocessable_entity }
      end
    end
  end

  # DELETE /administrators/1
  # DELETE /administrators/1.xml
  def destroy
    @administrator = Administrator.find(params[:id])
    @administrator.destroy

    respond_to do |format|
      format.html { redirect_to(administrators_url) }
      format.xml  { head :ok }
    end
  end
  
  # Login
  def login
    p cookies[:token]
    if params[:commit]
      #dbから探す
      @administrator = Administrator.find :first, :conditions => {
        :email => params[:administrator][:email],
        :password => params[:administrator][:password]
      }
      
      if @administrator
        #居た場合
        #token生成
        token_string = UUIDTools::UUID.timestamp_create().to_s
        
        cookies[:token] = token_string
        @administrator.token = token_string
        @administrator.save!
        redirect_to :action => :index
      else
        flash.now[:notice] = "Login Error."
        @administrator = Administrator.new(params[:administrator])
      end
    else
      @administrator = Administrator.new()
    end
  end
  
  def logout
    @administrator = Administrator.find_by_token cookies[:token]
    if @administrator
      @administrator.token = nil
      @administrator.save
    end
    cookies[:token] = nil
    
    redirect_to :action => :login
  end
  
  def login_required
    if cookies[:token]
      @administrator = Administrator.find_by_token cookies[:token]
      if @administrator
        p "login_required / OK"
      else
        p "login_required / NG"
        redirect_to :action=>:login
      end
    else
      p "login_required / NG"
      redirect_to :action=>:login
    end
  end
  
  def authorize
    p 'authorize'
  end
end
